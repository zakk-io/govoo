...
| --- | --- | --- |
| TC-SEC-001 | Fresh install; inspect `res.groups` | All nine groups from `security/access-control.md` §1 exist with the specified XML IDs (six base + three `govoo_contracts` addendum groups) |
...
| TC-SEC-010 | Contract Viewer Portal user attempts to navigate directly by ID/URL to a contract where they are neither counterparty nor named approver | Denied at record-rule/token level, not merely absent from their list (BR-CM-007, mirrors TC-SEC-005) |
| TC-SEC-011 | Any user, including Contract Manager and Board Administrator, attempts `write()`/`unlink()` on a `govoo.contract` once `state = 'executed'` | Rejected in all cases at the access-rights layer (BR-CM-004, mirrors TC-SEC-006) |
